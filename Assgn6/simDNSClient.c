...
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        // Construct query
        char query[4*sizeof(char) + sizeof(int)*n + sizeof(char)*sumOfLengths];
        query[0] = (qid >> 8) & 0xFF;               // Query ID Upper 8 bits
        query[1] = qid & 0xFF;                      // Query ID Lower 8 bits
        query[2] = 0x00;                            // Query type: Query
        query[3] = n;                               // Number of query strings
        int i=4,j=0;
        while(j < n){
            // Length of domain
        ...
        memcpy(packet + sizeof(struct ethhdr) + sizeof(struct iphdr), query, 4*sizeof(char)+sizeof(int)*n+sizeof(char)*sumOfLengths);
        ...
        queryNode->id = qid;
        queryNode->len = 4*sizeof(char)+sizeof(int)*n+sizeof(char)*sumOfLengths;
        queryNode->n = n;
        queryNode->tries = 1;
        ...
        int retval = select(sockfd+1, &readfds, NULL, NULL, &timeout);          // Wait for response
        if(retval == -1){
            perror("select");
            exit(1);
        }
        else if(retval == 0){
            // Timeout, increment tries and retransmit query
            queryNode->tries++;
